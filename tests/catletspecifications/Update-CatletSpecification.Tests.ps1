...
Describe "Catlet-Specifications" {
  ...
  Context "Update-CatletSpecification" {
    It "Updates catlet specification and sets new name with JSON config" {
      $config = @"
{
...
"@
      ...
      $updatedName = New-CatletSpecificationName
      $updateConfig = @"
{
  "name": "$updatedName",
  "parent": "dbosoft/e2etests-os/base",
...
"@
      $updatedSpecification = Update-CatletSpecification -Id $specification.Id -Comment 'second version' -Config $updateConfig
      $versions = Get-CatletSpecificationVersion -SpecificationId $specification.Id
      $versions | Should -HaveCount 2

      $latestVersion = Get-CatletSpecificationVersion -SpecificationId $specification.Id -Id $versions[1].Id
      $latestVersion.Id | Should -Be $updatedSpecification.Latest.Id
      $latestVersion.Comment | Should -Be 'second version'
      $latestVersion.Configuration.ContentType | Should -Be 'application/yaml'
      $latestVersion.Configuration.Content | Should -be $updateConfig.ReplaceLineEndings("`n")
      $latestVersion.Variants | Should -HaveCount 1
      $latestVersion.Variants[0].Architecture | Should -Be 'hyperv/amd64'
    ...
    It "Updates catlet specification and sets new name with YAML config" {
      $config = @"
name: $specificationName
parent: dbosoft/e2etests-os/base
variables:
